...
function parser(source) {
  ...
  while (readIndex < length) {
    ...
    if (rule.fenceStart.test(line)) {
      ...
      token +=
        `<pre class='line-numbers' ><code class='language-${language}'>${tempStr}</code></pre>`;
      readIndex++;
      continue;
    }
...
function inlineParser(p) {
  ...
  if (rule.inlinelink.test(p)) {
    p = p.replace(rule.inlinelink, function ($0, $1, $2, $3) {
      var href = /<(\S*)>/.exec($2) ? /<(\S*)>/.exec($2)[1] : $2;
      var title = /[('"](\S*)[)'"]/.exec($3)
        ? /[('"](\S*)[)'"]/.exec($3)[1]
        : "";
...
function Rules() {
  ...
  this.linkdefinedStart =
    /^\s{0,3}\[([\s]*[^\s].*)\]:\s*([^\s]*)(?:(?:\s+)(?:'.*'|"(.*)"))?(\s*)$/; // 链接定义 start
